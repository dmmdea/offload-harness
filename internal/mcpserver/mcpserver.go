...
func (s *Server) Run(ctx context.Context, version string) error {
	...
	srv.AddTool(&mcp.Tool{
		...
		InputSchema: json.RawMessage(`{"type":"object","properties":{"goal":{"type":"string","description":"the task for the local agent to accomplish"},"read_root":{"type":"string","description":"absolute directory the agent may read; it cannot read outside it (default: the server working dir)"},"max_steps":{"type":"integer","description":"hard step budget (default 12)"},"model":{"type":"string","description":"planner model id; must support tool-calling (default offload-e4b)"},"timeout_sec":{"type":"integer","description":"wall-clock budget in seconds (default 180)"}},"required":["goal"]}`),
	}, s.handleAgentRun)
...
func (s *Server) handleAgentRun(ctx context.Context, req *mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	model := in.Model
	if model == "" {
		model = "offload-e4b" // a tool-calling planner on llama-swap; matches the CLI default
	}
