...
// ---- bare-engine pins (endpoint is an engine, not llama-swap) ----------------

// bareVLLMServer: what a harness sees when `endpoint` is a vLLM port directly.
// Every /upstream/... path 404s (there is no proxy), /v1/models carries the
// engine's own rows (no meta at all on vLLM 0.28.0), /version answers.
func bareVLLMServer(t *testing.T, served, root string, maxLen int) *httptest.Server {
	t.Helper()
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Path {
		case "/v1/models":
			_ = json.NewEncoder(w).Encode(map[string]any{"object": "list", "data": []any{
				map[string]any{"id": served, "object": "model", "owned_by": "vllm", "root": root, "max_model_len": maxLen}}})
		case "/version":
			_ = json.NewEncoder(w).Encode(map[string]any{"version": "0.28.0"})
		default:
			http.NotFound(w, r)
		}
	}))
...
// bareLlamaServer: llama-server on its own port. Its /v1/models row DOES carry a
// `meta` (vocab/size facts) — but no `llamaswap` key — and /props answers at the
// root, describing the single model the process serves.
func bareLlamaServer(t *testing.T, served string) *httptest.Server {
	t.Helper()
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Path {
		case "/v1/models":
			_ = json.NewEncoder(w).Encode(map[string]any{"object": "list", "data": []any{
				map[string]any{"id": served, "object": "model", "owned_by": "llamacpp",
					"meta": map[string]any{"vocab_type": 2, "n_vocab": 151936, "n_ctx_train": 262144, "n_params": 9e9}}}})
		case "/props":
			_ = json.NewEncoder(w).Encode(propsFixture())
		default:
			http.NotFound(w, r)
		}
	}))
...
// llamaSwapUnknownModelServer: REAL llama-swap that does not know `model`. The
// passthrough 404s for it, the root roster is llama-swap's (meta.llamaswap on
// every row) and a bare /props would answer for WHATEVER seat is warm. The
// only correct pin here is no pin.
// rosterID is the ONE entry llama-swap lists, so that neither the name-match
// nor the sole-entry attribution rule can be what refuses — only the
// meta.llamaswap guard stands between the request and the warm seat's /props.
// (A first draft listed two unmatched entries; the multi-entry rule refused it
// and the guard test passed with the guard disabled.)
func llamaSwapUnknownModelServer(t *testing.T, rosterID string) *httptest.Server {
	t.Helper()
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Path {
		case "/v1/models":
			_ = json.NewEncoder(w).Encode(map[string]any{"object": "list", "data": []any{
				map[string]any{"id": rosterID, "object": "model",
					"meta": map[string]any{"llamaswap": map[string]any{"aliases": []string{}, "type": "model"}}}}})
		case "/props":
			_ = json.NewEncoder(w).Encode(propsFixture()) // whatever seat is WARM — must never be attributed
		default:
			http.NotFound(w, r)
		}
	}))
...
// A harness pointed straight at a vLLM port used to get NO pin: the whole probe
// spoke llama-swap's /upstream/ passthrough. The 2026-09-08 A2 re-run recorded
// four arms with seat_config_* absent on a build that had the vLLM probe, for
// exactly this reason.
func TestProbeSeatPinBareVLLMEndpoint(t *testing.T) {
	srv := bareVLLMServer(t, "qwen3.5-9b-vllm", "/hf/hub/models--RedHatAI--Qwen3.5-9B-quantized.w4a16/snapshots/a398088", 131072)
	defer srv.Close()
	pin, ok := ProbeSeatPin(context.Background(), srv.URL, "qwen3.5-9b-vllm")
	if !ok {
		t.Fatal("a bare vLLM endpoint produced no pin")
	}
	for _, want := range []string{"vllm 0.28.0", "served=qwen3.5-9b-vllm", "max_model_len=131072"} {
		if !strings.Contains(pin.Basis, want) {
			t.Errorf("basis %q missing %q", pin.Basis, want)
		}
	}
	// Same engine, same weights, same window, reached through llama-swap's
	// passthrough instead: the pin must be IDENTICAL, or a paired experiment
	// would refuse to compare a seat with itself over the path it was reached by.
	ver, models := vllmFixture("0.28.0", "/hf/hub/models--RedHatAI--Qwen3.5-9B-quantized.w4a16/snapshots/a398088", "qwen3.5-9b-vllm", 131072)
	viaSwap := vllmServer(t, "qwen3.5-9b-vllm", ver, models)
	defer viaSwap.Close()
	pin2, ok := ProbeSeatPin(context.Background(), viaSwap.URL, "qwen3.5-9b-vllm")
	if !ok || pin2.SHA256 != pin.SHA256 {
		t.Errorf("bare vs passthrough pins differ for the same served config: %q vs %q", pin.SHA256, pin2.SHA256)
	}
}

func TestProbeSeatPinBareLlamaServerEndpoint(t *testing.T) {
	srv := bareLlamaServer(t, "qwen3.5-9b-llamacpp")
	defer srv.Close()
	pin, ok := ProbeSeatPin(context.Background(), srv.URL, "qwen3.5-9b-llamacpp")
	if !ok {
		t.Fatal("a bare llama-server endpoint produced no pin")
	}
	if strings.HasPrefix(pin.Basis, "vllm ") {
		t.Errorf("llama-server pinned as vllm: %q", pin.Basis)
	}
	// Byte-identical to the passthrough pin of the same /props: one server, one hash.
	via, ok := probeFixturePin(t, propsFixture())
	if !ok || via.SHA256 != pin.SHA256 {
		t.Errorf("bare vs passthrough llama.cpp pins differ: %q vs %q", pin.SHA256, via.SHA256)
	}
}

// THE guard this fallback lives or dies by. llama-swap that does not serve the
// model must yield no pin — its root /props describes whichever seat is warm,
// and attributing that to `model` is the mis-attribution the file's first line
// refuses. The roster's meta.llamaswap is how the two cases are told apart.
func TestProbeSeatPinRefusesBareProbeOnLlamaSwap(t *testing.T) {
	// Sole entry that IS the requested name: the name rule would admit it.
	srv := llamaSwapUnknownModelServer(t, "seat")
	defer srv.Close()
	if pin, ok := ProbeSeatPin(context.Background(), srv.URL, "seat"); ok {
		t.Fatalf("llama-swap's warm seat was pinned via the bare path (name match): basis %q", pin.Basis)
	}
	// Sole entry that is NOT the requested name: the sole-entry rule would admit it.
	srv2 := llamaSwapUnknownModelServer(t, "the-only-configured-seat")
	defer srv2.Close()
	if pin, ok := ProbeSeatPin(context.Background(), srv2.URL, "seat-llama-swap-never-heard-of"); ok {
		t.Fatalf("llama-swap's warm seat was pinned via the bare path (sole entry): basis %q", pin.Basis)
	}
}

// A bare engine listing several names with none matching is ambiguous: refuse.
func TestProbeSeatPinBareMultiEntryNoMatchRefuses(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Path {
		case "/v1/models":
			_ = json.NewEncoder(w).Encode(map[string]any{"object": "list", "data": []any{
				map[string]any{"id": "alias-a", "root": "/hf/A", "max_model_len": 4096},
				map[string]any{"id": "alias-b", "root": "/hf/A", "max_model_len": 4096}}})
		case "/version":
			_ = json.NewEncoder(w).Encode(map[string]any{"version": "0.28.0"})
		default:
			http.NotFound(w, r)
	...
	defer srv.Close()
	if _, ok := ProbeSeatPin(context.Background(), srv.URL, "seat"); ok {
		t.Fatal("ambiguous bare roster produced a pin")
	}
}
