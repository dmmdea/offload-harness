...
func TestAdaptClassify_ThinkSpanStripped(t *testing.T) {
	...
	req := core.Request{Task: core.TaskClassify, Params: map[string]any{"labels": []string{"bug", "feature"}}}
	raw := "<think>Maybe {\"label\": \"feature\"}? No — it reports breakage.</think>\n{\"label\": \"bug\"}"
	...
	if m["label"] != "bug" {
		t.Fatalf("think-span fragment won over the real answer: got %q", m["label"])
	}
}
...
func TestWrapRunTier_UnsupportedTaskSkipsWithoutChat(t *testing.T) {
	...
	if stats.Unadaptable != 1 || stats.Remote != 0 {
		t.Fatalf("un-promptable task must count as unadaptable with no remote call: %+v", stats)
	}
}
