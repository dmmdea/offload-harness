...
type SpecBasis struct {
	...
	// artifact as one rendered by 0.118.0) but it is NOT what decides staleness:
...
func BasisOf(p Params) ParamsBasis {
	return ParamsBasis{
		...
		Seats: p.Seats, Home: p.Home, GOOS: p.GOOS, GPUEnv: p.GPUEnv,
...
func (b ParamsBasis) Params() Params {
	return Params{
		...
		Seats: b.Seats, Home: b.Home, GOOS: b.GOOS, GPUEnv: b.GPUEnv,
...
func AgainstRender(text string, freshBasis SpecBasis, freshBody string) Report {
	...
	if !ok {
		return Report{State: StateUnstamped, Detail: "no provenance block: rendered before serving-config stamping (0.118.0), or written by hand. `local-offload install render` stamps it"}
	}
