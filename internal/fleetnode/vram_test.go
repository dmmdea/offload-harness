...
// Real qube UUIDs (per the coordinator's UUID-pin follow-up, live-verified
...
func TestParseSmiMemoryDevices_QubeShape(t *testing.T) {
	in := "0, " + qube5060TiUUID + ", NVIDIA GeForce RTX 5060 Ti, 16311, 867\n" +
		"1, " + qube5070TiUUID + ", NVIDIA GeForce RTX 5070 Ti, 16303, 2187\n"
	devices, err := ParseSmiMemoryDevices(in)
	if err != nil {
		t.Fatalf("ParseSmiMemoryDevices: %v", err)
	}
	...
	if devices[0].Index != 0 || devices[0].UUID != qube5060TiUUID || devices[0].Name != "NVIDIA GeForce RTX 5060 Ti" {
		t.Errorf("devices[0] = %+v", devices[0])
	}
	if devices[1].Index != 1 || devices[1].UUID != qube5070TiUUID || devices[1].Name != "NVIDIA GeForce RTX 5070 Ti" {
...
// qube: pinning primary_gpu_uuid to the RTX 5070 Ti (the real compute card,
// smaller total by 8 MiB) must win over the RTX 5060 Ti even though
// HeadlineDevice alone would pick the 5060 Ti (see the QubeShape test above).
...
func TestSelectHeadlineDevice_PinnedUUIDBeatsLargerTotal(t *testing.T) {
	devices := []GPUDevice{
		{Index: 0, UUID: qube5060TiUUID, Name: "NVIDIA GeForce RTX 5060 Ti", TotalGiB: 16311.0 / 1024, FreeGiB: (16311.0 - 867) / 1024},
		{Index: 1, UUID: qube5070TiUUID, Name: "NVIDIA GeForce RTX 5070 Ti", TotalGiB: 16303.0 / 1024, FreeGiB: (16303.0 - 2187) / 1024},
	}
	head, warning := SelectHeadlineDevice(devices, qube5070TiUUID)
	if warning != "" {
		t.Fatalf("warning = %q, want none (UUID was found)", warning)
	}
	if head.Index != 1 || head.UUID != qube5070TiUUID || head.Name != "NVIDIA GeForce RTX 5070 Ti" {
...
func TestSelectHeadlineDevice_UUIDNotFoundFallsBackAndWarns(t *testing.T) {
	devices := []GPUDevice{
		{Index: 0, UUID: qube5060TiUUID, Name: "NVIDIA GeForce RTX 5060 Ti", TotalGiB: 16311.0 / 1024, FreeGiB: 15},
		{Index: 1, UUID: qube5070TiUUID, Name: "NVIDIA GeForce RTX 5070 Ti", TotalGiB: 16303.0 / 1024, FreeGiB: 13},
	}
...
func TestSelectHeadlineDevice_UnsetUUIDUnchanged(t *testing.T) {
	devices := []GPUDevice{
		{Index: 0, UUID: qube5060TiUUID, Name: "small-total-wise-bigger", TotalGiB: 16311.0 / 1024, FreeGiB: 15},
		{Index: 1, UUID: qube5070TiUUID, Name: "5070 Ti", TotalGiB: 16303.0 / 1024, FreeGiB: 13},
	}
...
func TestSelectHeadlineDevice_PinnedUUIDOnSingleGPUBox(t *testing.T) {
	devices := []GPUDevice{{Index: 0, UUID: qube5060TiUUID, Name: "solo", TotalGiB: 16, FreeGiB: 15}}
	head, warning := SelectHeadlineDevice(devices, qube5060TiUUID)
	if warning != "" {
...
func TestParseSmiMemoryDevicesSingleLine(t *testing.T) {
	devices, err := ParseSmiMemoryDevices("0, " + qube5060TiUUID + ", NVIDIA GeForce RTX 3070, 8192, 1024\n")
	if err != nil {
		t.Fatalf("ParseSmiMemoryDevices: %v", err)
	}
	if len(devices) != 1 {
...
func TestParseSmiMemoryDevicesMalformedLinesSkipped(t *testing.T) {
	in := "\n0, " + qube5060TiUUID + ", NVIDIA GeForce RTX 3070, 8192, 1024\n" +
		"not,a,valid,line,at,all,extra\n\n" +
		"1, " + qube5070TiUUID + ", NVIDIA GeForce RTX 4090, 24576, 2048\ngarbage\n"
	devices, err := ParseSmiMemoryDevices(in)
	if err != nil {
		t.Fatalf("ParseSmiMemoryDevices: %v", err)
	}
...
func TestParseSmiMemoryDevicesCRLF(t *testing.T) {
	in := "0, " + qube5060TiUUID + ", NVIDIA GeForce RTX 3070, 8192, 1024\r\n" +
		"1, " + qube5070TiUUID + ", NVIDIA GeForce RTX 4090, 24576, 2048\r\n"
	devices, err := ParseSmiMemoryDevices(in)
	if err != nil {
		t.Fatalf("ParseSmiMemoryDevices: %v", err)
...
// headline it over the larger-total device — the same qube-shaped override
...
func TestStartDeviceProbeSamplerPinnedUUID(t *testing.T) {
	...
	s := StartDeviceProbeSampler(ctx, 10*time.Millisecond, probe, qube5070TiUUID)
	snap, ok := s.Load()
	if !ok {
		t.Fatal("Load() not ok after start")
	}
