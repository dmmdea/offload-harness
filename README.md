...
| `offload_upscale_image` | `image`, `scale?`, `width?`, `height?`, `method?`, `model?`, `out?` | **ESRGAN enlarge** — AI-upscale a local image on the local ComfyUI with this machine's `upscale_model` (falls back to `videogen_upscale_model`; e.g. `4x-UltraSharp.pth`) → `{image_path, model, width, height}`, or defer. Default output is the model's native factor; `scale` rescales that result (2 on a 4x model = 4x then 0.5 lanczos), `width`+`height` pin it exactly. Synthesizes detail — an enlargement tool, not a faithful restore; for an exact resample use `offload_edit_image`'s `resize`. Takes the GPU lock (render is seconds; a cold ComfyUI start adds ~1-2 min). |
