...
- **MCP-native** — exposes 26 tools over stdio for any MCP client (Claude Code and friends), including a read-only local agent (`agent_run`). That is the count every box registers; a box with the delegator role enabled adds `agent_delegate`, and a box listing an accelerator adds its own — 11 for `hailo-8l`, 4 for `coral-edgetpu`, shared names registered once (ADR 0037), and a box listing a device it lacks in `fleet_accelerators` adds that device's tools as forwarders to the fleet node that has it, image bytes travelling with the call (ADR 0038, 0.115.0) — read `tools/list`, never a written figure.
- **Beyond text** — local **vision** (VQA / OCR / image-field-extract / render-QA), **speech-to-text** (whisper.cpp), **image/audio/video generation** (HiDream-O1 / SDXL / Qwen-Image · Wan 2.2 · Chatterbox TTS · ACE-Step via ComfyUI), **three image-edit routes** (deterministic PIL ops · masked inpainting · maskless instruction edits), and a dependency-free **SVG data-viz kit**.
...
offload-harness generate-image "a glass perfume bottle" --family qwen-image-2.1 --transparent   # a NAMED opt-in family (ADR 0058); non-commercial families return license-tagged results
...
# Compose (free, local CPU — HyperFrames HTML/CSS motion graphics; no GPU lock)
offload-harness compose-video --template title-card --variables '{"title":"Launch day","accent":"#22c55e"}'
offload-harness compose-video --template lower-third --format webm --variables-file lt.json --snapshots 1,2.5   # alpha overlay
offload-harness compose-video --html card.html --quality draft   # an inline single-file composition (trusted code only)
...
`generate-image --batch jobs.jsonl` renders every job through **one warm ComfyUI session** — the checkpoint loads once instead of once per image, and the zero-always-warm teardown (free VRAM, kill the spawned ComfyUI, release the GPU lock) runs at the **batch boundary**. One JSON object per line; `prompt` is required, everything else optional (`out`, `negative`, `width`, `height`, `steps`, `seed`, `refine` — a missing seed is minted, a missing out defaults under the media dir, and `"refine": false` opts one job out of a configured prompt refiner; `--refine=false` on the command opts out every job without its own value):
...
|---|---|---|
| `offload_status` | `section?` | **Capability discovery — call first when inspecting.** The LOCAL model roster (workhorse/agent/triage/escalation/reasoning/vision/ocr/stt/stt_hq/embed) + live served models from the local endpoint + this machine's media engines + the (only) remote surface. Everything except `offload_nim` runs on these local models. No argument returns everything, unchanged. `section:"brief"` is the form to call before sizing a contract: the `fleet` block plus one-line `gpu_lease_verdict` and `local_verdict` (about a quarter of the full answer). Any block name (`local`, `media`, `remote`, `accelerators`, `reuse`, `fleet`, `kv_cache_server`, `gpu_lease`) returns that block alone. An unknown value defers and lists the valid ones. On a COMPOSITE box (ADR 0052) it also reports `tier_profile`, `tiers` and one `layers[]` row per device layer — the seats, their live occupancy, and whether the layer would admit work right now — plus the same rows for every fleet node that publishes them. |
...
| `offload_generate_image` | `prompt`, `negative?`, `width?`, `height?`, `steps?`, `seed?`, `out?`, `refine?`, `family?`, `transparent?` | Generate an image on the local GPU (ComfyUI; this machine's configured model at its highest-quality settings — e.g. HiDream-O1 bf16 via its official family graph at native 2048, or SDXL on smaller boxes) → `{image_path, width, height, seed, family, ...}` (width/height MEASURED from the file), or defer. `family` selects a named opt-in binding (`offload_status` `media.image_families`); a non-commercial family (e.g. Qwen-Image-2.1, Qwen Research License) returns `license`, `commercial_use:false` and `license_note` — research/evaluation only (ADR 0058). `transparent` keeps an alpha channel (qwen-image-2.1 families only). Quality-first: renders may take minutes by design. Where `imagegen_refiner_model` is configured, the prompt is first expanded on the free local text tier (double-quoted spans guarded both ways; any refiner problem falls back to the raw prompt); `refine:false` renders the prompt verbatim. |
...
| `offload_animate_character` | `ref`, `driver`, `prompt`, `motion_prompt?`, `frames?`, `seed?`, `out?` | Retarget a driver video's motion onto a reference character image (WAN-Animate-2 distilled, one 81-frame chunk, keeps the driver's audio) on the local GPU → `{video_path, seed}`, or defer. |
...
| `offload_upscale_image` | `image`, `scale?`, `width?`, `height?`, `method?`, `model?`, `out?` | **ESRGAN enlarge** — AI-upscale a local image on the local ComfyUI with this machine's `upscale_model` (falls back to `videogen_upscale_model`; e.g. `4x-UltraSharp.pth`) → `{image_path, model, width, height, factor}`, or defer. Default output is the model's own factor; `scale` is the overall factor relative to the source, made exact (the source is measured and the output size pinned, so it holds for any model); `width`+`height` pin it yourself (≤ 16384). The written file's size is verified against the request and `factor` is the measured ratio (`factor_x`/`factor_y` for a non-uniform pinned size). Synthesizes detail — an enlargement tool, not a faithful restore; for an exact resample use `offload_edit_image`'s `resize`. Takes the GPU lock (render is seconds; a cold ComfyUI start adds ~1-2 min). |
...
| `offload_edit_image_generative` | `image`, `prompt`, `negative?`, `preset?`, `steps?`, `cfg?`, `seed?`, `out?`, `family?`, `images?`, `transparent?` | **Maskless instruction edit** — rewrite a local image from a text instruction on the local ComfyUI (Qwen-Image-Edit class, `gen_edit_*` binding): "make it snowing heavily", "turn the leather into fur" — the model reads the source through its own vision encoder and re-renders the whole frame → `{image_path, seed}`, or defer. Pick among the three edit routes by what you have: `offload_edit_image` for deterministic ops, `offload_inpaint_image` when you can draw a mask (only the masked region is re-imagined; the rest is preserved in intent but not pixel-identical — `grow_mask` feathers 16 px by default and the frame is VAE round-tripped), this when the change is global or diffuse. Output follows the source resolution within 0.9-2.0 MP (`gen_edit_megapixels` pins it); `preset` is a matched steps+cfg+LoRA triple (`lightning8` default · `full` · `lightning4`). Every ≥16 GB ComfyUI tier seeds the Qwen-Image-Edit 2511 binding (since 0.132.5); elsewhere the route defers until a machine binds it. `family` selects a named edit family: a qwen-image-2.1 family takes `images` (up to 9 references after the target, 10 in all) and `transparent`, and returns license-tagged results (non-commercial). |
...
| `offload_compose_video` | `template`+`variables?` \| `html` \| `project_dir`, `composition?`, `format?`, `fps?`, `quality?`, `resolution?`, `workers?`, `strict?`, `snapshots?`, `out?` | **Designed motion graphics to video** with HyperFrames (pinned, env-scrubbed; ADR 0059): a vetted template (`title-card`, `lower-third`) filled with typed variables, or a trusted inline/local composition → MP4 / WebM (VP9 alpha) / MOV (ProRes 4444 alpha) / GIF / PNG sequence. Gated by `lint` + `check` + an ffprobe measurement → `{video_path\|frames_dir, duration_sec, fps, frames, width, height, has_alpha, has_audio, codec, render_ms, lint, check, snapshots}`, or a typed defer (`BAD_INPUT`, `LINT_ERRORS`, `CHECK_FAILED`, `RENDER_FAILED`, …). CPU-class — software GL + CPU encode, never takes the GPU lock; local only, never cloud. `html`/`project_dir` are trusted code (Chrome runs without a sandbox); the fleet task accepts templates only. |
...
| `agent_run` | `goal`, `read_root?`, `max_steps?`, `model?`, `timeout_sec?`, `profile?`, `judge?`, `context_class?` | **Local read-only agent.** A local model plans and iterates over read-only tools (`list_dir`, `read_file`) + the `offload_*` cascade to do a bounded multi-step read-and-reason job → `{output, steps, stop_reason, tools, effects?, effects_flagged?, judge_report?}`, or defer. `profile` narrows the tool list per task shape; `judge: true` adds one end-of-run **advisory** same-seat completion grading the run's flagged effects for operator review (never gates anything). No writes, no shell, no network; ledger untouched. |
| `offload_ask` | `question`, `paths[]`, `read_root?` | **One-call delegation.** Ask a bounded question ABOUT SPECIFIC FILES and a free local seat answers it — the one-call form of `agent_delegate`. You supply only question + paths; the harness builds the whole contract (goal, `{answer,evidence}` output schema, and an acceptance check anchored to distinctive tokens mined from the files themselves) and runs it on the local agent seat → `{answer, evidence, verified, acceptance, seat, steps, stop_reason, cache_hit}`, or defer. An IDENTICAL repeat within one session — same question, same `read_root`, same file **bytes** — is served from an in-process cache without spending the seat again and reports `cache_hit: true` — the seat is not re-run on that call. It is keyed on CONTENT, so editing an attached file makes the next call a fresh run and a stale answer cannot be served. Not a general speedup: a *different* question over the same files still pays full seat time. Reach for it the moment you are about to open more than two files to answer something bounded. Files are inlined by the harness under `read_root` (<=16 files, <=128 KiB each, <=256 KiB total), so the caller context never pays for them. REFUSES when the files hold no token distinctive enough to ground the check, rather than returning an answer nothing verified. |
| `offload_review_diff` | `diff`\|`diff_path`, `task`, `max_findings?`, `read_root?` | **Clean-context review.** Review a code diff on a free local seat that sees ONLY the diff and the task statement, never this session's history — the isolation is the mechanism: a reviewer without the author's accumulated context catches defects the author's own judgement has stopped seeing → `{findings:[{severity,file,line,claim,why}], reviewed_bytes, seat, steps, stop_reason, note?, dropped_ungrounded?, dropped_echo?, truncated_by_cap?}`, or defer. `note` explains an EMPTY list in words; the three counts say what is *not* in the list (invented path · the prompt's own template echoed back · hidden by your `max_findings`). A zero-finding result whose raw answer does not read as an explicit clean verdict DEFERS rather than reporting an empty list — a broken run must never arrive looking like a clean diff. **Advisory only**: never gates a merge, never substitutes for the final does-it-actually-work verification. Findings are triage input, not verdicts — read the flagged lines and decide; an empty list means this reviewer found nothing, not that the change works. Findings naming a file the diff never touched are dropped and counted. Caps: 10 findings, 256 KiB inline / 128 KiB via `diff_path`. |
| `offload_classify_image` | `image_path`, `domain?`, `top_k?` | Classify an image on the local Coral Edge TPU → `{results:[{label,score}],best,model,domain}`; `domain` = `imagenet` (default) \| `birds` \| `insects` \| `plants`. *Accelerator-gated: registered only on a box listing `coral-edgetpu`.* |
| `offload_semantic_segment` | `image_path`, `out_path?` | Semantic segmentation (DeepLabV3 MNv2, 21 Pascal classes) on the local Coral Edge TPU; writes a per-pixel class-id PNG → `{mask_path,classes:[{class_id,label,pixels}],width,height}`. Not instance masks (see `offload_segment`). *Accelerator-gated (`coral-edgetpu`).* |
...
| `offload_object_detect` | `image_path`, `score_threshold?`, `size?` | Detect the 80 COCO classes on the local accelerator → `{objects:[{label,class_id,x,y,w,h,score}],count}` — YOLOv8s with on-chip NMS on `hailo-8l`, EfficientDet-Lite0/1/2 (`size`) on `coral-edgetpu`; one owner per box, the first listed (ADR 0037). *Accelerator-gated.* |
...
| `offload_image_embed` | `image_path` | Image embedding for similarity search / clustering on the local accelerator → `{embedding,dim,space}` — 512-d TinyCLIP on `hailo-8l`, 1280-d EfficientNet-EdgeTPU-S (`space:"efficientnet-edgetpu-s"`, not CLIP) on `coral-edgetpu`; one owner per box (ADR 0037). *Accelerator-gated.* |
| `offload_pose` | `image_path`, `score_threshold?` | Human pose (YOLOv8s-pose, host-decoded raw head) on the local NPU: 17 named COCO keypoints per person → `{people:[{x,y,w,h,score,keypoints:{nose:{x,y,score},…}}],count}`. *Accelerator-gated.* |
| `offload_segment` | `image_path`, `everything?`, `score_threshold?`, `out_path?` | Instance segmentation (YOLOv8s-seg 80 classes; `everything:true` = FastSAM class-agnostic) on the local NPU; writes an instance-id mask PNG → `{instances:[{label,class_id,x,y,w,h,score}],mask_path,count}`. *Accelerator-gated.* |
| `offload_text_embed` | `text`, `space?` | Text embedding computed ON the local NPU in the SAME space as `offload_image_embed` (`tinyclip`, 512-d, default; `siglip2` 768-d pairs with the zero-shot siglip2 side) → `{embedding,dim,space}`. *Accelerator-gated.* |
| `offload_zero_shot` | `image_path`, `labels`, `space?`, `template?` | Zero-shot classification: score an image against free-text labels, BOTH towers on the local NPU → `{results:[{label,similarity,prob}],best}`. *Accelerator-gated.* |
...
- **`search_files`** — a ripgrep-style regex search over the worktree (`os.Root`-confined). Output is grouped per file with line numbers and **hard-capped at 100 matches**; a `mode: "files"` variant returns only the paths that contain a match, and a `glob`/`path` narrows the search. It lets a small model *find* code by matching lines instead of reading whole files. Uses `rg` when it's on PATH, else a confined Go walk — identical output either way. The pattern is **case-sensitive**; `(?i)` prefixes it for a case-insensitive match, and a zero-match result says so explicitly and names that retry — a small planner that gets a bare "no matches" tends to retry with a *longer* query and then conclude the text is absent.
...
**Two-tier mode** (`--two-tier`, Task C8): an architect/editor split following aider's one-shot handoff. The planning model (`--architect-model`, default `gemma4-26b-a4b`) drafts one complete, standalone plan using read/search tools only; a separate edit model (`--editor-model`, default `offload-e4b`) then executes that plan as its **sole** instruction — it never sees the original request or any history. On a single GPU this is exactly one cold model swap (plan-once, not per-step alternation). A degenerate/empty plan falls back to a single-model run of the original objective. `--two-tier` conflicts with any **non-default** `--profile` (two-tier sets the architect/editor toolsets itself); `--profile general` or an empty value coexists.
...
qwen-image preset knobs and the opt-in `imagegen_refiner_model` prompt refiner), `gen_edit_*`,
`inpaint_*`, `videogen_*`, `voicegen_*`, the named license-tagged families (`imagegen_families`,
`gen_edit_families` — ADR 0058), the ComfyUI launch profile (`comfy_cuda_device`,
`comfy_dynamic_vram`, `comfy_extra_args`) and the composition lane's `compose_*` / `hyperframes_*`
keys (ADR 0059) — are documented in
...
`tools/` carries **printed CLIs** — standalone Go tools generated by
[CLI Printing Press](https://github.com/mvanhorn/cli-printing-press) and vendored here as repo
tooling. Each is its own Go module, so it stays out of the harness's dependency graph and out of a
root `go build ./...`.
...
- **A tighten-only risk-rule table sits inside the policy broker.** Structural rules (action kind +
  a path/host glob — never shell-command patterns, which the OS cage owns) can **deny or ask, never
  allow**; a built-in floor denies secret-material paths (`.env*`, `*.pem`, `*.key`, `id_rsa*`,
  `id_ed25519*`) whatever the flags say. Load a versioned JSON table with `--rules`; the audit trail
