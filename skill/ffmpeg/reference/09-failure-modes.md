...
| # | Symptom | Cause | Fix |
|---|---|---|---|
| 1 | drawtext renders nothing, exit 0, stderr "Stray % near …" [measured 8.1.2] — on **9.0/9.0.1 the same input exits −22 with no output file** [measured 2026-09-10] | any `%` in `text=`/`textfile=` under `expansion=normal` (`%%` too) | `expansion=none`, or split into two drawtext instances; pixel-diff the frame (mandatory on 8.x, where it is silent) |
| 2 | "No option name near '/Windows/Fonts/…'" [measured 8.1.2 + 9.0] | `fontfile=C\:/…` unquoted — or quoted with the colon left unescaped (`'C:/…'`), which fails the same way | `fontfile='C\:/…'` (quoted **and** escaped) or `C\\:/…`; never the drive-less form (row 3a); see 08 |
| 3 | ffmpeg crashes 0xC0000005 / exit 139 after "Fontconfig error: Cannot load default config file" [measured 8.1.2 + 9.0] | `font=`/no fontfile, or an unparsable fontfile path falling back to fontconfig | always `fontfile=` with a valid path; libass (`subtitles`/`ass`) is unaffected (DirectWrite) |
| 3a | The SAME drawtext command draws on one box and segfaults on another [measured 2026-09-10] | `fontfile=/Windows/Fonts/…` is drive-less → resolved against the **current drive**; the rig's `TEMP` is `D:\Temp` and `D:\Windows\Fonts` does not exist, so it falls back to fontconfig (row 3) | use `fontfile='C\:/Windows/Fonts/…'`; never a drive-less font path in a pipeline whose cwd you do not control |
...
| 17 | `av1_nvenc`: "No capable devices found" [measured edge node] | Ampere (RTX 30xx) has no AV1 encoder; encoder is listed anyway | h264/hevc_nvenc, or CPU libsvtav1; probe with a 1-frame test per host |
| 18 | ffprobe on animated WebP: "image data not found", 0×0 [measured **8.1.2 only**] | no animated-webp demuxer in 8.x | verify with PIL (`n_frames`, `is_animated`) on 8.x. **9.0/9.0.1 probe it natively** — `codec_name=webp_anim` with real dimensions (`nb_frames` still N/A) [measured 2026-09-10] |
...
| 28 | ffmpeg ran, output present, but the wrong ffmpeg [measured path facts] | workstation has 8.1.2 on PATH and 9.0.1 in `<dev>\tools` (harness lane) | print `ffmpeg -version` in logs; pin the path in scripts |
| 29 | laptop/editing rig unreachable over Tailscale [measured 2026-09-01] | hosts off (last seen hours ago) | `tailscale status`; do not conclude a build fact from an offline box — dump when up |
| 30 | Delegated doc digests came back about "Go versions" or timed out [measured 2026-09-01] | 27B seat stuck in llama-swap `starting` (429 on every call); edge node 4B answered off-goal to generic `offload_research` contracts | `agent_delegate` with the file named in the goal + page-only acceptance tokens works on the edge node; fetch trac with `curl -A "Go-http-client/1.1"` (Anubis only challenges browser UAs); read ffmpeg.org sections directly |
