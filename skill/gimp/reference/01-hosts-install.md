...
| `plug-ins\gimp-mcp-plugin\gimp-mcp-plugin.py` | the only user plug-in installed (copy of `<dev>\tools\gimp-mcp\gimp-mcp-plugin.py`) |
...
| Tool | Where | Use |
|---|---|---|
| `ffprobe` | on PATH (Gyan ffmpeg 8.1.2 winget build); harness also has `<dev>\tools\ffmpeg-9.0.1\bin` | codec, WxH, pix_fmt for png/jpg/webp/tif/gif/bmp/avif/heic/jxl/jp2/tga/exr/qoi/psd/ico |
...
### Related installs
- gimp-mcp source: `<dev>\tools\gimp-mcp` (git remote github.com/maorcc/gimp-mcp, HEAD 09bfb2d
...
- On-demand MCP launcher: `%USERPROFILE%\.claude\mcp-ondemand\mcp-ondemand.ps1` + `stash.json`
  (gimp-mcp stashed as user-scope stdio: `uv run --directory <dev>/tools/gimp-mcp gimp_mcp_server.py`).
...
| gimp-mcp | **not installed** — no plug-in in the profile, no `<dev>\tools\gimp-mcp` source. MCP mode (02 §C) is workstation-only |
