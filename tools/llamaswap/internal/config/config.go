...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		// 127.0.0.1 literal, never the hostname: Windows resolves localhost to
		// ::1 first and the proxy binds IPv4, costing ~21s per call in stalls.
	...
	if v := os.Getenv("LLAMASWAP_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("LLAMASWAP_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "llamaswap-pp-cli", "config.json"), nil
}
