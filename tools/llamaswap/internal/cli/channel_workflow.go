...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  llamaswap-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  llamaswap-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"models", "profiles"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  llamaswap-pp-cli workflow status

  # Show status as JSON
  llamaswap-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("llamaswap-pp-cli")
			}
