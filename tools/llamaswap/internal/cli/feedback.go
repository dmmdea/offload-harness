...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LLAMASWAP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LLAMASWAP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "llamaswap-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// feedback takes FREE-FORM text: every positional is valid by
		// construction, so there is no invalid-argument error path to probe.
		// The one refusal is empty text, which a positional probe cannot
		// express. Without this the live dogfood asserts a non-zero exit for
		// an argument the command is right to accept — and writes its probe
		// string into the operator's feedback ledger to do it.
...
When ` + "`LLAMASWAP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LLAMASWAP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  llamaswap-pp-cli feedback list
  llamaswap-pp-cli feedback list --limit 5
  llamaswap-pp-cli feedback list --json`,
