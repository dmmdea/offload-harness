...
type rootFlags struct {
	...
	// invocation. Mirrors the LLAMASWAP_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "llamaswap-pp-cli",
		Short: "The llama-swap operations console — durable history, drain-aware control, and measurement commands with three specific guarantees: keep-set unloads are refused statically by id AND alias (never from server ttl), `--drain` fails closed when slot state is unreadable, and fit/ctx refuse to answer inside their uncertainty band instead of guessing.",
		Long: `Command a llama-swap fleet from the shell — model roster, swap state, drain-aware unloads, live metrics, and every llama.cpp per-model endpoint — with a durable local record of swaps and speeds the server forgets on restart.
...
Health check: run 'llamaswap-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("llamaswap-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'llamaswap-pp-cli profile list')")
	...
	rootCmd.AddCommand(newServerCmd(flags))
	rootCmd.AddCommand(newUpstreamCmd(flags))
	...
	rootCmd.AddCommand(newInflightPromotedCmd(flags))
