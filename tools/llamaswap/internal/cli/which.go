...
var whichIndex = []whichEntry{
	{Command: "keepset audit", Description: "See when the memory-stack models were actually resident, every eviction attributed to its cause, and how much embed/rerank traffic landed inside a degraded window.", Group: "Local state that compounds", WhyItMatters: "Reach for this after any restart or seat change to prove the memory stack never silently degraded."},
	{Command: "seat log", Description: "A per-model chronology of every flag change mined from the dated config-backup series, with the YAML comments as the reasoning and the live cmd as the current truth.", Group: "Local state that compounds", WhyItMatters: "Answers 'when did this seat get that flag and what else landed in the same edit' without opening 19 backups by hand."},
	{Command: "sync", Description: "Pull the server's in-memory request activity and event stream into local SQLite before a restart destroys them, sealing each server epoch and quantifying what the ring buffer already dropped.", Group: "Local state that compounds", WhyItMatters: "Run it before asking any historical question; every other analytics command reads what sync stored."},
	{Command: "swaps", Description: "Cold-load percentiles per model, time lost to swapping, and which model pairs repeatedly evict each other.", Group: "Local state that compounds", WhyItMatters: "Use it to decide seat groupings and TTLs from measured cost instead of intuition."},
	{Command: "verify", Description: "Embeds a fixed probe on the embedder and reranks a fixed pair on the reranker, asserting both against stored calibrated baselines — catching dropped flags that roster counting cannot.", Group: "Guardrails with teeth", WhyItMatters: "Run after every config change or restart; it is the step of the manual ritual that has historically caught real outages."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  llamaswap-pp-cli which "stale tickets"
  llamaswap-pp-cli which "bottleneck"
  llamaswap-pp-cli which --limit 1 "send message"
  llamaswap-pp-cli which                                # list the full capability index`,
