...
Run sync first to populate the local search index.`,
  ...
  llamaswap-pp-cli search "status"

  # Force local search explicitly
  llamaswap-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  llamaswap-pp-cli search "status" --type activity --data-source local
  # JSON output for piping
  llamaswap-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'llamaswap-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
