...
import (
	...
	"llamaswap-pp-cli/internal/learn"
	"llamaswap-pp-cli/internal/learn/lookups"
...
type syncResult struct {
	...
	// SoftWarnings counts sync_warning events the resource emitted while
	// still completing successfully (resource_not_incremental,
	// max_pages_cap_hit, ...). Soft-warned resources keep their success
	// classification for exit-code purposes but are reported as "warned"
	// in the run summary so the summary never contradicts the event
	// stream printed above it.
	SoftWarnings int
}

// syncWarningCountingWriter counts sync_warning event lines written through
// it. Every event is emitted as one complete Fprintf line, so substring
// counting per Write is exact; one instance wraps the shared event writer per
// resource, inside that resource's single worker goroutine.
type syncWarningCountingWriter struct {
	w io.Writer
	n int
}

func (c *syncWarningCountingWriter) Write(p []byte) (int, error) {
	c.n += strings.Count(string(p), `"event":"sync_warning"`)
	return c.w.Write(p)
}
...
func newSyncCmd(flags *rootFlags) *cobra.Command {
	...
	// Spine (wave A) additions: the epoch-aware activity/event mirror. The
	// framework resource sync above copies REST resources; the mirror copies
	// the proxy's in-memory activity ring and event stream, which are the only
	// data on this box that a restart destroys.
	var mirrorWatch bool
	var mirrorSeal bool
	var mirrorInterval time.Duration
	var mirrorEventWindow time.Duration
	var mirrorOff bool
...
Exit codes & warnings:
  ...
  run. Each is emitted as a {"event":"sync_warning",...} line carrying
  status, reason, and message fields, and a final
  {"event":"sync_summary",...} aggregates the run (its "warned" count
  includes resources that synced successfully but emitted a warning).

  Stream placement: with --json, the event lines (sync_start,
  sync_progress, sync_complete, sync_warning, sync_anomaly, and the
  summary when the mirror pass runs) stream as NDJSON on STDERR, and
  stdout carries exactly one JSON document — the mirror report, or the
  sync_summary line when --no-mirror is set — so piping stdout to a
  JSON parser (jq, ConvertFrom-Json, json.loads) parses cleanly.
  Without --json the NDJSON stream stays on stdout.
...
Activity mirror (epoch-aware):
  After the resource sync, this command mirrors the proxy's IN-MEMORY
  activity ring and drains /api/events into the local domain tables.
  That history is the only data here a proxy restart destroys, so the
  mirror maintains a restart-epoch lifecycle (open -> sealed) rather
  than appending blindly:

    * Restart witness. /api/metrics/stats total_requests is read BEFORE
      and AFTER each activity fetch. A decrease across that fence means
      the batch straddles two server processes: it is discarded whole,
      the open epoch is sealed, and the poll retries. A restart that
      races past the previous cursor (higher ids AND a higher counter)
      is caught by comparing mirrored row fingerprints at overlapping
      ids.
    * Sealing marks that epoch's non-terminal requests CENSORED, not
      failed. A request still running when the process died has no
      observable outcome, and the censored set is biased toward long
      requests.
    * Loss is reported as three quantities, never one: loss_evicted
      (exact only while activity ids stay dense — density is asserted
      every batch, and a violation sets ids_dense=0 with loss NULL,
      never a guessed number), loss_prepoll (counter-derived), and the
      post-poll tail, which is permanently UNKNOWABLE and is printed as
      "unknowable" rather than 0.
    * The epoch count is a LOWER BOUND: two restarts between two polls
      read as one boundary.

  --no-mirror skips this pass. --watch runs the mirror alone in a
  foreground loop. --seal asserts a boundary by hand.`,
		Example: `  # Sync all resources
  llamaswap-pp-cli sync
  # Sync specific resources only
  llamaswap-pp-cli sync --resources models,profiles

  # Full resync (ignore previous checkpoint)
  llamaswap-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  llamaswap-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  llamaswap-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  llamaswap-pp-cli sync --latest-only

  # Mirror the activity ring and drain events, then report epochs and loss
  llamaswap-pp-cli sync --json

  # Foreground poll loop (no daemon); Ctrl-C stops it cleanly
  llamaswap-pp-cli sync --watch --interval 30s

  # Force-seal the open epoch (operator asserts the boundary; no network read)
  llamaswap-pp-cli sync --seal`,
		RunE: func(cmd *cobra.Command, args []string) error {
			// Spine short-circuits run BEFORE the resource sync: --seal is a
			// pure local assertion and --watch is a mirror-only poll loop, so
			// neither should re-page the REST resources on every tick.
			mirrorOpts := spineMirrorOptions{DBPath: dbPath, EventWindow: mirrorEventWindow}
			if mirrorSeal {
				return spineRunSeal(cmd, flags, dbPath)
			}
			if mirrorWatch {
				return spineRunWatch(cmd, flags, mirrorOpts, mirrorInterval)
			}
			...
			// In --json mode the progress/diagnostic events (sync_start,
			// sync_progress, sync_complete, sync_warning, sync_anomaly)
			// stream as NDJSON on stderr so stdout carries exactly one
			// parseable JSON document: the mirror report, or the
			// sync_summary line when --no-mirror is set. Without --json the
			// NDJSON stream stays on stdout (the piped-mode contract).
			...
			for i := 0; i < concurrency; i++ {
				...
				go func() {
					...
					for resource := range work {
						events := &syncWarningCountingWriter{w: syncEventWriter}
						res := syncResource(cmd.Context(), c, db, resource, sinceTS, full, maxPages, effectiveLatestOnly, prune, userParams, events)
						res.SoftWarnings = events.n
						results <- res
					}
				}()
			}
			...
			// Soft-warned resources stay "success" for exit-code purposes but
			// count as warned in the summary, so the summary agrees with the
			// sync_warning events emitted above it.
			summaryWarned := warnCount + softWarnedCount
			summarySuccess := successCount - softWarnedCount
			if humanFriendly {
				if summaryWarned > 0 {
					fmt.Fprintf(os.Stderr, "Sync complete: %d records across %d resources (%d warned, %.1fs)\n",
						totalSynced, totalResources, summaryWarned, elapsed.Seconds())
				} else {
					fmt.Fprintf(os.Stderr, "Sync complete: %d records across %d resources (%.1fs)\n",
						totalSynced, totalResources, elapsed.Seconds())
				}
			} else {
				// Under --json with the mirror pass enabled, the summary joins
				// the event stream on stderr and the mirror report is the one
				// stdout document. With --no-mirror the summary line IS the
				// stdout document.
				summaryWriter := syncEventWriter
				if flags.asJSON && mirrorOff {
					summaryWriter = cmd.OutOrStdout()
				}
				fmt.Fprintf(summaryWriter, `{"event":"sync_summary","total_records":%d,"resources":%d,"success":%d,"warned":%d,"errored":%d,"duration_ms":%d}`+"\n",
					totalSynced, totalResources, summarySuccess, summaryWarned, errCount, elapsed.Milliseconds())
			}
			...
			// Spine mirror pass: page the activity ring under the epoch
			// contract, drain /api/events briefly, and report epochs + loss.
			// Runs after the resource sync so a mirror failure cannot mask a
			// resource-sync result the operator already saw.
			if !mirrorOff {
				if err := spineRunMirror(cmd, flags, spineMirrorOptions{DBPath: dbPath, EventWindow: mirrorEventWindow}); err != nil {
					return err
				}
			}
			return nil
	...
	cmd.Flags().BoolVar(&mirrorWatch, "watch", false, "Foreground poll loop for the activity mirror only (no resource sync, no daemon). Ctrl-C stops it cleanly.")
	cmd.Flags().DurationVar(&mirrorInterval, "interval", 15*time.Second, "Poll interval for --watch.")
	cmd.Flags().BoolVar(&mirrorSeal, "seal", false, "Force-seal the open restart epoch and exit. Non-terminal requests in that epoch are marked censored, not failed. No network read.")
	cmd.Flags().DurationVar(&mirrorEventWindow, "event-window", 2*time.Second, "How long to drain /api/events per pass (0 disables the event drain).")
	cmd.Flags().BoolVar(&mirrorOff, "no-mirror", false, "Skip the activity/event mirror pass; run only the framework resource sync.")
