...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  llamaswap-pp-cli doctor
  llamaswap-pp-cli doctor --json
  llamaswap-pp-cli doctor --fail-on warn
  llamaswap-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "llamaswap-pp-cli", suggestion)
						} else {
			...
			// PATCH (wave D, see .printing-press-patches/internal-cli-doctor.go.md):
			// llama-swap-specific deployment checks. Every check lives in
			// doctor_extras.go; this is the only hook the generated doctor needs.
			// The findings land under report["llamaswap"] as a map with a
			// "status" key, which doctorExitForFailOn already knows how to read.
			runDoctorExtras(cmd.Context(), flags, report)
			...
			// PATCH (wave D): human rendering for the llama-swap deployment
			// findings written above. No-op when the section is absent.
			renderDoctorExtras(w, report)
			return doctorExitForFailOn(failOn, report)
		},
	}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'llamaswap-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'llamaswap-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'llamaswap-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'llamaswap-pp-cli sync' to refresh."
	}
	return report
}
