...
func withJournalHome(t *testing.T) string {
	...
	t.Setenv("LLAMASWAP_NO_LEARN", "")
	t.Setenv("LLAMASWAP_LEARN_NO_CAPTURE", "")
	t.Setenv("LLAMASWAP_LEARN_SESSION", "")
	return dir
}
...
func TestJournal_SessionKeyFromEnv(t *testing.T) {
	withJournalHome(t)
	t.Setenv("LLAMASWAP_LEARN_SESSION", "session-abc123")
	if err := runCLI(t, "version"); err != nil {
		t.Fatalf("version: %v", err)
	}
	entries := readAllJournalEntries(t)
...
func TestJournal_DisableSwitchesWriteNothing(t *testing.T) {
	cases := []struct {
		...
		{name: "master no-learn env", env: map[string]string{"LLAMASWAP_NO_LEARN": "1"}},
		{name: "capture-only env", env: map[string]string{"LLAMASWAP_LEARN_NO_CAPTURE": "1"}},
