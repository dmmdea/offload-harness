# Patch: `internal/cli/upstream_lora_adapters.go`
...
Nothing else in the file changed: /upstream/{model}/lora-adapters returns JSON, so the generated read path
is correct here.
...
`upstream lora-adapters --model example-value`, which llama-swap correctly refused with
...
never be auto-started or evicted by being read. Loaded LoRA adapters and their scales. Live-verified 2026-08-13: embeddinggemma answers 200 (empty list).
