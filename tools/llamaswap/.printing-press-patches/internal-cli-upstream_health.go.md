# Patch: `internal/cli/upstream_health.go`
...
Nothing else in the file changed: /upstream/{model}/health returns JSON, so the generated read path
is correct here.
...
`upstream health --model example-value`, which llama-swap correctly refused with
...
never be auto-started or evicted by being read. Per-model readiness. Live-verified 2026-08-13: embeddinggemma answers 200.
