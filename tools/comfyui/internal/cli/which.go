...
var whichIndex = []whichEntry{
	{Command: "submit", Description: "Submitting a graph that is already rendering attaches to the running job instead of starting a second one.", Group: "Local state that compounds", WhyItMatters: "Reach for this instead of POSTing /prompt directly whenever a render may already be in flight; a duplicate submit costs a full GPU render."},
	{Command: "models why", Description: "Says why ComfyUI cannot see a model file, separating four causes that every other tool collapses into one.", Group: "Answers the server cannot give", WhyItMatters: "Use this the moment a loader rejects a filename, before re-downloading anything."},
	{Command: "nodes options", Description: "Prints the values a loader input will actually accept, reading both /object_info spec shapes.", Group: "Answers the server cannot give", WhyItMatters: "Check a filename here before spending a render on a value the validator will reject."},
	{Command: "set", Description: "Applies overrides to a graph's inputs, refusing to write when a node no longer holds the class it did when the address was captured.", Group: "Reproducible edits", WhyItMatters: "Use instead of hand-editing graph JSON; the class assertion is what makes a template update fail loudly rather than silently."},
	{Command: "validate", Description: "Checks a graph against the cached node schema before submitting — unknown classes, missing required inputs, and COMBO values that are not members of the option list.", Group: "Reproducible edits", WhyItMatters: "Run before every submit; a rejection caught here costs nothing, one caught by the server has already consumed a queue slot."},
	{Command: "timing", Description: "Reports duration from the server's own execution timestamps, compared against the historical distribution for the same graph shape.", Group: "Local state that compounds", WhyItMatters: "Use this rather than reading the server log when comparing configurations; log scraping has produced a false regression before."},
	{Command: "sync-history", Description: "Copies the server's in-memory render history into the local store before a restart destroys it.", Group: "Local state that compounds", WhyItMatters: "Run before restarting ComfyUI, or the runs you are comparing disappear."},
	{Command: "exp", Description: "Runs a parameter sweep as one durable object, with failed arms recorded as first-class rows rather than gaps.", Group: "Local state that compounds", WhyItMatters: "Use for any comparison across settings; an OOM is a result worth keeping, not an absence."},
	{Command: "provenance", Description: "Given a produced file, reports the run, graph, experiment arm, timing, server identity, and staged input that made it.", Group: "Answers the server cannot give", WhyItMatters: "Use when a rendered file needs to be traced back to the exact configuration that produced it."},
	{Command: "stage", Description: "Stages a host file into ComfyUI's input directory and records its content hash, so archived runs stay reproducible.", Group: "Reproducible edits", WhyItMatters: "Stage inputs rather than copying them by hand when a comparison needs to hold for months."},
	{Command: "replay", Description: "Re-runs a stored graph and reports a before/after delta including which server configuration each ran under.", Group: "Local state that compounds", WhyItMatters: "Use to confirm a change actually moved the number, rather than comparing against a stale baseline."},
	{Command: "outputs", Description: "Lists produced files and, with --probe, fills in resolution, frame rate, duration, and whether audio is present.", Group: "Answers the server cannot give", WhyItMatters: "Use when comparing video models; the server cannot tell you whether a clip has sound."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  comfyui-pp-cli which "stale tickets"
  comfyui-pp-cli which "bottleneck"
  comfyui-pp-cli which --limit 1 "send message"
  comfyui-pp-cli which                                # list the full capability index`,
