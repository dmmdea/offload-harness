...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  comfyui-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  comfyui-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  comfyui-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"history", "objectinfo"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  comfyui-pp-cli workflow status

  # Show status as JSON
  comfyui-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("comfyui-pp-cli")
			}
