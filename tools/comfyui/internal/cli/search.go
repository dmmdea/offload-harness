...
Run sync first to populate the local search index.`,
  ...
  comfyui-pp-cli search "status"

  # Force local search explicitly
  comfyui-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  comfyui-pp-cli search "status" --type history --data-source local
  # JSON output for piping
  comfyui-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'comfyui-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
