...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "comfyui-pp-cli",
		Short: `Drive a local ComfyUI render server from the shell, with a durable record of every run the server itself forgets.`,
		Long: `Drive a local ComfyUI render server from the shell, with a durable record of every run the server itself forgets.
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'comfyui-pp-cli profile list')")
	...
	rootCmd.AddCommand(newObjectinfoCmd(flags))
	rootCmd.AddCommand(newQueueCmd(flags))
	rootCmd.AddCommand(newUserdataCmd(flags))
	...
	rootCmd.AddCommand(newPromptPromotedCmd(flags))
	rootCmd.AddCommand(newSystemPromotedCmd(flags))
	rootCmd.AddCommand(newUploadPromotedCmd(flags))
	rootCmd.AddCommand(newViewPromotedCmd(flags))
