...
func writeAPIErrorEnvelope(w io.Writer, flags *rootFlags, err error, code int) {
	...
	// Rich envelope shape (errenvelope.go) instead of the bare {error, code}
	// pair, and mark structuredWritten so the end-of-run envelope goes to
	// stderr instead of appending a second JSON document to stdout.
...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			"\n      Run 'comfyui-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. This API is configured without credentials, so the service may be blocking the request by rate limit, geography, bot protection, or endpoint policy."+
			"\n      Run 'comfyui-pp-cli doctor' to check connectivity.", err))
...
func wrapPlatformStructuredOutput(data json.RawMessage, flags *rootFlags, resultKey string, mergeOwnedEnvelope bool) (json.RawMessage, error) {
	// Mark BEFORE the nil-session early return: the mark tracks "a structured
	// document is going to stdout", which is true whether or not a platform
	// session exists. Over-marking is the safe direction — it moves the error
	// envelope to stderr, whereas under-marking would put two JSON documents
	// on stdout. (errenvelope.go)
	if flags != nil {
		flags.structuredWritten = true
	}
...
func printOutputWithFlagsMeta(w io.Writer, data json.RawMessage, flags *rootFlags, agentMeta map[string]any, documentedFields ...map[string]bool) error {
	if flags != nil && platformStructuredOutputSelected(w, flags) {
		flags.structuredWritten = true // errenvelope.go: a structured document is going to stdout
	}
