...
// collectionItemsForOutput was removed as dead code: paginatedGet already inlines
// the same envelope projection (paginatedCollectionEnvelopeField, then
// extractPaginatedItems), so the wrapper had no call site anywhere in this CLI.
// Both helpers it delegated to remain live.
