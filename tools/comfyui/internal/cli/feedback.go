...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("COMFYUI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("COMFYUI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "comfyui-pp-cli/feedback")
...
When ` + "`COMFYUI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`COMFYUI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  comfyui-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  comfyui-pp-cli feedback --stdin < notes.txt
  comfyui-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  comfyui-pp-cli feedback list
  comfyui-pp-cli feedback list --limit 5
  comfyui-pp-cli feedback list --json`,
