# ComfyUI Printed CLI Agent Guide

This directory is a generated `comfyui-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
comfyui-pp-cli doctor --json
comfyui-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
comfyui-pp-cli which "<capability>" --json
comfyui-pp-cli <command> --help
```

Add `--agent` to command invocations for JSON, compact output, non-interactive defaults, and no color:

```bash
comfyui-pp-cli <command> --agent
```
...
```bash
comfyui-pp-cli <command> --help
comfyui-pp-cli <command> --dry-run --agent
```
...
Disable the loop with `--no-learn` per-invocation or `COMFYUI_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
