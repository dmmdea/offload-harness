...
func main() {
	...
	maxSameTool := fs.Int("max-same-tool", 0, "cap on calls to any one tool name per run — the circuit breaker for a model that loops instead of progressing (e.g. repeated/reworded web_search calls). Negative disables the cap; 0 (default) uses the loop's built-in default (8 since 0.79.0; the exact-repeat refusal is the real loop guard).")
