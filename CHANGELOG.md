...
- Tests: 55 pass. Five mutants, each confirmed to typecheck first, are all caught — including one
  ...
  expectations).
## [0.134.0] - 2026-09-22 - offload_status answers one block, or the brief form

### Added

- **`offload_status` takes one optional argument, `section`.** The full answer is ~18 KB (4.5k tokens
  ...
  `fleet` block plus one-line `gpu_lease_verdict` and `local_verdict`: 4,443 bytes against 17,927
  on the reference box (24.8%). A block name (`local`, `media`, `remote`, `accelerators`, `reuse`,
...
- **A wrong call is a defer, not the full dump.** An unknown section (`"gpu"`), an unknown argument
  ...
  every argument, so a guessed one silently cost the whole 18 KB.
