...
- **An auto contract is polled at the node's sized wall, not at the wire cap** (register D-116). Since D-03 a contract
  ...
  - **The delegator sizes its poll clock from what the node advertises.** `delegate.autoPollBound(view, contract)` builds
    a `seatrate.SeatPolicy` from the target node's health `seat_rate` + `seat_budget` and runs `seatrate.AutoWallFor` —
    the SAME function `pipeline.AutoWallFor` runs on the node, over the same `seatrate.FinalBudgets` / `seatrate.InputFor`
    rule, clamped to the same `[AgentTimeoutSecDefault, AgentTimeoutSecCap]`. One arithmetic, two sources of numbers; a
    test sizes one contract through both entry points and fails if they disagree. A node advertising no usable rate is
    still polled to the cap — unchanged behaviour, and the reason says so.
  - **The node publishes the wall it is RUNNING under.** `GET /fleet/jobs/{id}` carries `wall_sec` (additive,
    ...
    `fleetnode.Jobs.SetWall`. The delegator RAISES its bound to that number the moment it sees one and never lowers it:
  ...
  - **The poll-deadline message names the bound's source** — `poll bound: sized from <node>'s seat_rate X tok/s (N
    samples): M s`, `poll bound: the node's own wall M s`, or `poll bound: cap: no seat rate advertised by <node>` — on
    the defer and on the never-owned FAILURE alike.
