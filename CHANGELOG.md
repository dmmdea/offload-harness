...
- **`/fleet/health` publishes six honest fields about work in flight** (registers S-17, S-36; all
  ...
  ONLY, never a path that could LOAD the seat — register C-05; a failed read leaves both ABSENT rather
  than publishing "not loaded"); `lease_exclusive` / `lease_draining` (what the held reservation is
