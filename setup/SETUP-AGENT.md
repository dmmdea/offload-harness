...
| `blackwell-16` | #1 (5060 Ti 16 GB) — MEASURED on <node-b> 2026-07-12..08-02 | `gemma4-26b-a4b` | 32K | q8_0 | full-GPU — CUDA-13 serves (slower); 12.8 for peak |
| `blackwell-3x16` | #17 (3x 16 GB Blackwell; the 3-card reference workstation) | `gemma4-26b-a4b` + the vLLM agent seat | 128K (long twin 256K) | q8_0 | COMPOSITE (ADR 0039): also a complete `blackwell-16` and `blackwell-2x16`; placement picks the layer per task; device 1 is the display card and its layer ships dormant |
| `volta-16` | #2 (V100 16 GB) | `gemma4-26b-a4b` | 32K | q8_0 | full-GPU |
| `ampere-16` | 3090-class ≥12 GB (defensive) | `offload-e4b` (agent `qwen38-27b-agent` = Qwen3.8-27B UD-IQ3_S + embedded MTP head, `--ctx-size 49152`, `research`; 26B dropped) | 49K | q8_0 | 26B dropped — MEASURED 2026-09-04 on an A2 16 GB @ 40 W: 26B-A4B 1/8, 4B 8/8. SEAT RE-AUDITED 2026-09-16 (ADR 0047): the 27B+MTP beats the 4B 24/24 blind; `--ctx-size 49152` is the measured fit (14,410 MiB of 15,356, including the resident embedder) and a larger window aborts at load |
...
> city96/ComfyUI-GGUF issue #247. Measured live on `blackwell-16` (<node-b>, then a single RTX 5060 Ti 16 GB) 2026-07-19 by the
...
PROJECTED notes with MEASURED values (same-PR docs), and this box joins the harness's
real-hardware validation set — seven tiers as of 2026-09-07 (`blackwell-2x16`, `blackwell-16`, `blackwell-8`, `ampere-8`, `ampere-6`, `ampere-16`, `amd-gcn`).
...
used where it is live today (<node-b>: `offload-12b`, also answering to `gemma4-12b-qat`; the
...
| `escalation_model` | `offload-12b` *(<node-b>-local — see note above)* (gemma-4-12B + MTP drafter) | **82.1 tok/s — 2.5× the 26B it offloads from**; grammar-clean 5/5; task-level A/B vs the both-26B incumbent showed zero regressions (outputs content-identical) |
...
inapplicable until the operator adds the 12B entry to llama-swap out-of-band (as <node-b> did:
gemma-4-12B QAT + MTP drafter, aliases `offload-12b`/`12b`/`quality`). Full validation record:
the operator's benchmark archive (`2026-07-20_<node-b>-roster-validation/`, ROSTER.md).
