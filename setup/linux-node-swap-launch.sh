...
STAGED="" TARGET="" SHA256="" BACKUP_SUFFIX="" HEALTH_URL="" CONFIG_PATH=""
RESTART_COMMAND="" RENDER_TARBALL="" RENDER_DIR=""
WAIT_IDLE_TIMEOUT="10m" VERIFY_TIMEOUT="90s"
DRY_RUN=0 SKIP_HASH_CHECK=0
RUNNER_EXE="" LOG_DIR="" NODE_SWAP_BIN=""
...
# RUNNER_EXE: the exe used to RUN `node-swap` itself. Defaults to TARGET, same
# reasoning as the Windows launcher — the currently installed binary performs its
# own replacement, matching every prior deploy record's pattern. Override only for
# a first-ever rollout of this tool onto a node whose CURRENT build predates
# node-swap (same "first-rollout override" case the d5207011 deploy record hit on
# every node in that deploy, none of which had node-swap yet).
if [ -z "$RUNNER_EXE" ]; then RUNNER_EXE="$TARGET"; fi
...
ARGS=(node-swap --staged "$STAGED" --target "$TARGET" --sha256 "$SHA256"
