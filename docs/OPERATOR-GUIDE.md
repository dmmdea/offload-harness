...
| **cold-load hold** (0.140.0) | the node, while a request may be waiting for its first byte (admission, prefill, re-pack): `/running` read every 5 s and before any stall is filed | a seat llama-swap is loading (row not `ready`, or absent while another model holds the cards), and the first completion after a load (the admission warm-up loaded it, or the probe saw the load): the stall clock stops and the run sits in phase `cold-load` under a ceiling of `max(600 s, 2 × the seat's measured cold_load_sec)` counted from the silence; the hold ends on the seat's first byte, not on `ready` alone (a fresh vLLM seat measured 60 s of first-request warm-up after reading `ready`); past the ceiling the run is filed `stalled: seat still loading after Xs in cold-load (…)` as **infrastructure** | an unreadable `/running` with no load seen (the prefill clock then runs as before) |
