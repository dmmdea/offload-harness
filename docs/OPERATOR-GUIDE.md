...
| **A config file fails to LOAD naming a port** | A configured HTTP base at `:0` or `:9` is refused by key and value — `:0` is the OS’s "any free port" and `:9` is IANA discard, so both are the shape of a value that was never substituted and both used to cost a full dial timeout per call (`endpoint`, `delegate_remotes[]`, `cascade_remote_lanes{}`, `seat_endpoints{}`, `fleet_queue_holder`, `tts_endpoint`, `nim_endpoint`, `hailo_endpoint`, `coral_endpoint`, `pair_workloads_endpoint`). A **loopback** base on an unusual port is still allowed — that is INV-10’s bench twin. |
