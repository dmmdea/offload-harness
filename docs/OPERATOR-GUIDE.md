...
| **cold-load hold** (0.140.0) | the node, while a request waits for its first byte: `/running` read every 5 s and before any prefill/re-pack stall is filed | a seat llama-swap is loading mid-run (row not `ready`, or absent while another model holds the cards): the stall clock stops and the run sits in phase `cold-load` under a ceiling of `max(600 s, 2 × the seat's measured cold_load_sec)`; past it the run is filed `stalled: seat still loading after Xs in cold-load (…)` as **infrastructure**; once the seat reads `ready` the prefill clock restarts with its full allowance | an unreadable `/running` (the prefill clock then runs as before) |
