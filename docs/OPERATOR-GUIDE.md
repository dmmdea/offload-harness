...
| delegator poll | `timeout_sec` + a grace window — for an UNSIZED (`timeout_auto`) contract, the bound the target node's advertised `seat_rate`/`seat_budget` imply, by the node's own arithmetic, raised to the node's published `wall_sec` while it runs, and the 900 s cap only when the node advertises no rate (D-116); C-27 credits back intervals the node PROVABLY spent queued (both endpoints observed `accepted`), bounded at `min(poll bound + grace, 5 min)`; a job that never started is a `queue deadline` FAILURE, never a `budget` defer | waiting for the node's answer | the capacity wait (`agent_placement_wait_sec`, `results[].capacity_wait_sec`) |
