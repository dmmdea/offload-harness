...
| `503 queue full (… limit N)` | `fleet_max_queue_depth` reached — default is now `2x fleet_max_concurrent_jobs` (8 with the default 4 workers), not a flat 32, so a busier node hits this sooner than it used to. The response carries a `Retry-After` header (and health carries the same number as `queue_wait_estimate_sec`) sized from that node's own `recent_agent_wall_sec` — honor it before retrying, or raise `fleet_max_queue_depth` if the box should hold a deeper backlog. Never size your own retry from `seat_rate.min_turn_sec`; that is a per-seat retry floor, not a queue-depth signal. |
