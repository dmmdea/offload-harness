...
| `seat_tok_s` | float | 0.115.21 (register D-03): this run's effective decode rate — completion tokens per second of call wall over the completions that generated ≥ 128 tokens (tool-call completions are prefill-dominated and excluded). 0/absent = no qualifying completion. Also the ledger row's `tok_per_s`. |
...
| `min_turn_sec` | int | 0.115.21: cold load + one turn at the final budget — the least wall a retry is worth on this seat; the delegator's retry floor is `max(agent_retry_min_sec, min_turn_sec)`. |
