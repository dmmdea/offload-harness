...
- **Ties keep the rotation** (the comparison is strict), so a fan-out of same-shaped contracts across equal seats still deals one each instead of stacking on one.
