...
| `spread` (0.79.0) | one `Run` fetches every remote's health ONCE, then deals the subtasks round-robin across the local seat AND every remote that passes the hard gate for that subtask — local, node A, node B, local, … — so an N-contract fan-out genuinely runs on N seats at the same time. Measured before it existed: `auto` put four concurrent contracts on one box, `remote` put four on the other one. No eligible remote → every subtask runs local and the reason says so. |
