...
| `agent loop: canceled (the caller's context ended)` | `budget` | 2026-09-17 (register S-22/W-16). The PARENT context was canceled mid-LOOP (the delegator abandoned the poll, the node is shutting down) — the mirror of the re-pack's own arm below, added to the loop branch which fell through to the generic `agent loop: …` infrastructure shape instead: the failed request looks exactly like a dial refusal (a `*url.Error`), so an operator used to be told to fix a box that never misbehaved (12 `agent loop: context canceled` rows filed as broken hardware). Nothing on the box failed; the caller went away. |
