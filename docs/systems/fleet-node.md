...
| `spread` (0.80.0) | one `Run` fetches every remote's health ONCE, then deals the subtasks round-robin across the local seat AND every remote that passes the hard gate for that subtask — local, node A, node B, local, … — so an N-contract fan-out genuinely runs on N seats at the same time. The deal is positional and deterministic (`placeSpread`: slot 0 is always the local seat, pinned by `TestRunSpreadDealsAcrossLocalAndEveryEligibleRemote`), so a 2-contract spread with an eligible remote is guaranteed one local + one remote — the pair shape. Per-subtask eligibility means a contract failing the gate (no `output_schema`, over-size) silently takes the local slot instead; `results[].placement` names where each landed. Measured before spread existed: `auto` put four concurrent contracts on one box, `remote` put four on the other one. No eligible remote → every subtask runs local and the reason says so. |
