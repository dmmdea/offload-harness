...
| `wall_sec` | int | 0.126.0 (register D-03): the wall the node ran under when IT sized it — a `timeout_auto` contract, the estimate below clamped to 300..900; `wall_note` is prefixed `auto wall (timeout_auto):`. Absent when the contract named its own `timeout_sec`, or when the seat had no rate yet and the wire default ran. |
