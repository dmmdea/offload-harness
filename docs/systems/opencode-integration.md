...
| Plugin | [dmmdea/opencode-local-offload](https://github.com/dmmdea/opencode-local-offload) (loader in `~/.config/opencode/plugins/`) | Protocol injection, `task` description rewrite, read-only task reroute, H14 nudge, delegate placement digest, agent/command provisioning, instrument, `offload_plugin_status` |
