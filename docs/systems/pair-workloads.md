...
| `scheduledOn` | the PAIR UUID of the harness node named in the placement, resolved by name from PAIR's `cluster/members.json` (harness node names equal PAIR member names — the hostnames each box advertises); the local UUID for local work or an unknown name |
