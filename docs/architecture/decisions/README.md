...
| [0050](0050-placement-ranks-adequate-seats-by-expected-completion.md) | Accepted | Placement (`route=auto`/`remote`) ranks quality-adequate seats by expected completion under the operator-signed INV-5 rider: a wall-time term enters only as (i) a feasibility floor from the contract's own FITTED final at the seat's measured rate — never its max-final `min_turn_sec` — and (ii) an ordering key among seats already past the capability/adequacy gate, with power-of-two-choices among near-ties (a seeded draw) so independent dispatchers do not herd; `route=auto` now reads the local seat's own in-flight count, not just the GPU lease; one joint deal per Run replaces N independent per-subtask probes and respects each node's headroom (`max_concurrent_jobs − jobs_running`, no floor); a declared-but-idle lease demotes a remote instead of excluding it, reserving hard exclusion for an exclusive/draining hold or a busy non-text lease; `placement_reason` narrates every reachable remote with a one-word verdict; the delegator long-polls (`?wait=12`) and honors a dispatch 503's `Retry-After`; `offload_status` publishes the same job-registry in-flight signal fleet-wide |
| [0051](0051-decision-margin-full-denominator-behind-a-flag.md) | Accepted | The decision margin is normalised over MATCHED class tokens only, which excludes mass on unmatched tokens and on legal labels outside the `top_logprobs` window and inflates the margin ~10x (declared mass measured at ~0.097 on llama.cpp), so the gate under-escalates on hard many-label contracts; the full denominator ships behind `confidence_margin_full_denominator` (default off) with its OWN `confidence_margin_threshold_full` (default 0 = the gate never fires; the matched-scale 0.65 is never applied to the full scale), every row carries `margin_scale` (empty = matched) plus `margin_declared_mass` and `margin_ambiguous` recorded in BOTH modes, and the three history-reading consumers (health baselines, exemplar harvest, conformal calibration) filter to one scale and report what they excluded |
| [0052](0052-a-box-is-the-union-of-its-tiers-and-placement-is-a-per-task-decision.md) | Accepted | A box is the union of its tiers, and placement is a per-task decision: `composes`/`layers`, one placement table, display-card guards that fail closed, `placed` on every result |
| [0053](0053-linux-amdgpu-gpu-memory-provider.md) | Accepted | The Linux GPU memory provider reads amdgpu sysfs (VRAM + the driver's GTT pool on UMA profiles); the generic source is named per OS (`linux-amdgpu` / `windows-generic`) — the seam ADR 0014 left open, closed by the first Linux AMD node |
| [0054](0054-dual-route-node-renders-a-cpu-seat-family-beside-its-gpu-seats.md) | Accepted | A dual-route node renders a CPU seat family beside its GPU seats: `alt_backends` declares it per tier, `install render --llama-bin-cpu` produces `<id>-cpu` entries with the cpu template's flags in the interactive set, the manifest and `/fleet/health` (`backends`) say what was rendered; the route is picked by seat id |
| [0055](0055-walls-are-ceilings-liveness-is-progress.md) | Accepted | Walls are ceilings, liveness is progress: every seat completion streams and each delta is a progress event; a contract is ended by a STALL (no progress inside the seat's dynamic per-phase allowance, filed as infrastructure) or by a safety CEILING (`max(3 × estimate, 2 × wall, 1800 s)`, filed as budget), never by its wall expiring while the seat is producing; the wall is the reported expectation; liveness telemetry rides the run registry, `/fleet/jobs/{id}` and the wire; the delegator polls while the node reports progress |
| [0056](0056-llamacpp-prompt-cache-tiers.md) | Accepted | llama.cpp prompt-cache tiers: the host-RAM `--cache-ram` is sized per RAM tier under a floor rule (never below llama-server's own 8192 default unless the RAM cannot carry it) and measured at 18x on a repeated context; the SSD slot save/restore lane ships on the node but its `--slot-save-path` flag is NOT rendered — a missing path stops llama-server from starting, and a restore buys zero prefill on b9934 (upstream #25913) — so the delegator half is blocked with an explicit unblock condition |
| [0057](0057-placement-scores-the-cards-the-harness-can-use.md) | Accepted | Placement scores the cards the harness can use, not the operator's screen: `gpu_util_pct` keeps its meaning (the busiest card, for the dashboard and PAIR) and a new additive `work_util_pct` skips a card reporting `display_active` via `gpuprobe.DisplayCardUUIDs` — the ONE rule the lease verdict also uses — with a single-GPU guard; the figure is chosen per NODE so the placement comparator stays a strict weak ordering |
| [0058](0058-non-commercial-model-families-ship-only-as-named-license-tagged-opt-ins.md) | Proposed | Non-commercial model families ship only as named, license-tagged opt-ins: `imagegen_families` / `gen_edit_families` overlays beside the default binding, each REQUIRING `license` + `commercial_use` (load refuses otherwise), selected per request by `family`; every result carries `family`/`license`/`commercial_use` (+ `license_note` when not commercial) and the ledger row the license; never a default, never a seed default — amends ADR 0011's reasoning for named, tagged families (first case: Qwen-Image-2.1, Qwen Research License) without editing it |
| [0059](0059-external-cli-media-tool-runs-cpu-class-pinned-env-scrubbed.md) | Proposed | An external-CLI media tool runs CPU-class, pinned, env-scrubbed and never via its cloud paths: HyperFrames (offload_compose_video) renders in software GL + CPU encode with no GPU lease (ADR 0026) on its own in-process slot; a project-local lockfile install checked by `npm audit signatures`; an allowlisted child env (no cloud key, NODE_OPTIONS or GPU_LEASE_*), a harness-owned HOME and a fresh cwd with no `.env`; only lint/check/render/snapshot/browser/--version, always `--json`; lint + check + an ffprobe gate with typed defers; the fleet door accepts vetted templates only (compositions are trusted code in an unsandboxed Chrome) |
| [0048](0048-vllm-is-a-first-class-engine-on-every-tier.md) | Accepted (Amendment 1, 2026-09-17: blackwell-16 pays its seat — measured on the RTX 5060 Ti, `kv_cache_dtype fp8` because fp8_e5m2 is NaN there under vLLM 0.29; WSL launches pin the V1 runner; coverage 4 of 16) | vLLM is a first-class engine on every tier, equal to llama.cpp, and a tier never loses it as a side effect: ADR 0047's ship deleted ampere-16's whole `vllm_seat` to change a MODEL, which also removed the tier's LMCache capability (a binding is per vLLM seat, ADR 0045) while both existing guards passed vacuously because they `continue` on a nil declaration; the seat is restored, `TestEveryTierCanSeatAModelUnderVLLM` asserts the SET of tiers (regression floor + a countable `vllmSeatDebt`, today 3 of 16), and `TestEveryDeclaredVLLMSeatValidates` fails instead of skipping when nothing is declared |
