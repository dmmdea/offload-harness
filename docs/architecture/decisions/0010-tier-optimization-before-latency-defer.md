...
- The predicted-latency-defer proposal that prompted this decision is **not adopted** as the remedy;
  its pull request is closed, and this ADR is the recorded outcome ("the task it became"). The idea is
  not foreclosed forever — the conditions under which it could be revisited are the ones above.
