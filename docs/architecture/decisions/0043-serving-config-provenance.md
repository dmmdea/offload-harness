...
- **UNSTAMPED does not exit 1.** Every config on the fleet today predates stamping (verified
  read-only on all three reference nodes at 0.118.0: Qube, Lenovo and Aorus all report
...
Live audit at 0.118.0, read-only, three nodes, all UNSTAMPED. Tests:
