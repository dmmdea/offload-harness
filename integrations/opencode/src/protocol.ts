...
export function protocolText(mcp: string, offloadAgent: string, primaryTools: "tier1" | "all" = "all", offloadTools: "recon" | "all" = "all"): string {
  const t = (name: string) => `${mcp}_${name}`;
  if (primaryTools === "tier1") return tier1ProtocolText(mcp, offloadAgent, offloadTools);
  return [
...
export function taskDescriptionAddendum(offloadAgent: string, mcp: string, primaryTools: "tier1" | "all" = "all", offloadTools: "recon" | "all" = "all"): string {
  if (primaryTools === "tier1") {
    // The primary cannot call agent_delegate in tier1; several legs = several offload task calls.
    if (offloadTools === "recon") {
      return `\n\nOFFLOAD ROUTE: read-only legs (recon, sweep, digest, extract, inventory, audit over LOCAL files) belong on subagent_type "${offloadAgent}" — a free local seat with the harness's read-and-digest lanes. Media legs (generate or edit images, video or audio; transcribe or describe audio/video) belong on subagent_type "${offloadAgent}-media". Several such legs: issue them as separate task calls in the SAME message. Keep judgment/web/write legs on the default agent.`;
    }
...
// to call a tool it cannot see. Everything else is reached by handing a task leg to an offload
// subagent. Shorter than the full text: it is paid every turn. With offloadTools "recon" the
// harness is split across two subagents, so the text says which legs go where — still naming no
// harness tool the primary cannot see.
function tier1ProtocolText(mcp: string, offloadAgent: string, offloadTools: "recon" | "all" = "all"): string {
  const t = (name: string) => `${mcp}_${name}`;
  const route =
    offloadTools === "recon"
      ? [
          `Everything else — multi-file recon, digests, audits, extraction over local files, image OCR/VQA, web research over given URLs, roster/health — goes to the local offload seat: issue a task call with subagent_type "${offloadAgent}" and hand it NAMED FILES and one bounded question. It holds the harness's read-and-digest lanes (agent_delegate, agent_run, research).`,
          `Media — generate or edit images, video or audio; transcribe or describe audio/video; image detection; the opt-in NIM surface — goes to task subagent_type "${offloadAgent}-media" (same local seat, the media tools).`,
        ]
      : [
